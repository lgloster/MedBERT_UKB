...
def write_EHRinstance_to_example_files(seqs,max_seq_length, max_predictions_per_seq,masked_lm_prob,vocab,rng):
...
#### That is the output I need
    ...
    # with open('UKB_feats.json','w') as f:
...
def main(_):
  ...
  for filename in os.listdir(curr_dir):
    ...
    if filename.startswith("MI_Trial"):
      
      name_split = filename.split(".")
      
      output_name = name_split[0] + "_" + name_split[2]
      ...
      write_EHRinstance_to_example_files(train_data,FLAGS.max_seq_length, FLAGS.max_predictions_per_seq,FLAGS.masked_lm_prob,vocab,rng)
      with open(f'{output_name}.json','w') as f:
          json.dump(dict_list, f)
      print("FINISHED:",output_name,len(dict_list))
